...
API надсилає лист після **реєстрації**, **повторного запиту** (`POST /customer/me/resend-verification`) і **відновлення пароля** (`POST /customer/auth/forgot-password`), якщо увімкнено SMTP.
...
**Render + Brevo:** якщо `SMTP_PORT=587` дає `Connection timeout`, використати `SMTP_PORT=2525`, `SMTP_SECURE=false`. Brevo підтримує 2525 як альтернативний SMTP submission port, коли хостинг блокує 587.
...
2. `SMTP_HOST=smtp-relay.brevo.com`, `SMTP_PORT=587` (`2525` на Render, якщо 587 timeout), `SMTP_SECURE=false`.
...
Тема: **Відновлення пароля — Malva Garden**  
Посилання: `{WEB_ORIGIN}/account/reset-password?token=…` (дійсне 1 год).
