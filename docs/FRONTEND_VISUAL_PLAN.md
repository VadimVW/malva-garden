...
| # | Етап | Мета |
|---|------|------|
| **14** | Полірування UI вітрини (баги + візуал) | ✓ — сітка каталогу, пошук, підменю навбара |
| **15** | Банери сторінок каталогу (адмінка) | ✓ |
| **16** | Hub `/catalog` (адмінка) | ✓ hub; шапка — далі |
...
**Зроблено:** `useFloatingListPosition` (dropdown пошуку); `NavSubmenuPanel` (loading + без іконок); сітка `auto-fill 225px` на `/catalog/*` і `/search`. Банери та метадані категорій — **етап 15**.

### Відомі проблеми (з коду, до етапу 15)
...
| `bannerTitle` | `String?` | Заголовок на банері (**необовʼязковий**; не fallback на `name`) |
| `bannerSubtitle` | `String?` | Підзаголовок на банері (необовʼязковий) |
...
- `MalvaGardenCatalogDesktop`: `bannerImageUrl`, `bannerTitle`, `bannerSubtitle`. Порожні обидва тексти → **лише зображення** (без білої плашки); `sectionTitle` — у h1 під банером.
...
- Категорія лише з `bannerImageUrl` (без текстів) — банер без білої обводки/плашки.
- Підкатегорії можуть мати власний банер або успадковувати (узгодити: **власний банер на slug** — простіше).

---

## Етап 16 — Hub `/catalog` + глобальні налаштування (в кінці)

**Узгоджено:** hub і контактні дані шапки — через адмінку **«Налаштування»** (`SiteSetting`), без redeploy після підключення ключів у вітрині. **`Category.imageUrl`** на вітрині не використовується (прибрати з форми категорії).
